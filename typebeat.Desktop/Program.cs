...
namespace typebeat.Desktop
{
    ...
    {
#if DEBUG
        private const string base_game_name = @"typebeat-development";
#else
        private const string base_game_name = @"typebeat";
#endif
        ...
        public static void Main(string[] args)
        {
                ...
                if (windowsVersion.Major < 6 || (windowsVersion.Major == 6 && windowsVersion.Minor <= 2))
                {
                    unsafe
                    {
                        ...
                        SDL3.SDL_ShowSimpleMessageBox(SDL_MessageBoxFlags.SDL_MESSAGEBOX_ERROR,
                            "Your operating system is too old to run type!beat"u8,
                            "This version of type!beat requires at least Windows 8.1 to run.\n"u8
                            + "Please upgrade your operating system or consider using an older version of type!beat.\n\n"u8
                            + "If you are running a newer version of windows, please check you don't have \"Compatibility mode\" turned on for type!beat"u8, null);
                        return;
                    }
                }
            }
            ...
            if (OperatingSystem.IsWindows() && !OsuGameDesktop.IsPackageManaged)
            {
                ...
                {
                    // Icon/association registration is cosmetic; never block the game on it.
                }
            }
            ...
            using (DesktopGameHost host = Host.GetSuitableDesktopHost(gameName, hostOptions))
            {
                if (!host.IsPrimaryInstance)
                {
                    ...
                    if (!DebugUtils.IsDebugBuild)
                    {
                        Logger.Log(@"type!beat does not support multiple running instances.", LoggingTarget.Runtime, LogLevel.Error);
                        return;
                    }
                }
