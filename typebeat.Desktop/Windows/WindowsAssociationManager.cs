...
namespace typebeat.Desktop.Windows
{
    ...
    public static class WindowsAssociationManager
    {
        ...
        /// </summary>
        private const string program_id_file_prefix = "typebeat.File";

        private const string program_id_protocol_prefix = "typebeat.Uri";

        private static readonly ApplicationCapability application_capability = new ApplicationCapability(@"typebeat", @"Software\typebeat\Capabilities", "type!beat");
        ...
        private static readonly FileAssociation[] file_associations =
        {
            ...
            new FileAssociation(@".typb", WindowsAssociationManagerStrings.OsuBeatmap, Icons.TypeBeat, claimDefault: true),
        ...
        private class FileAssociation
        {
            ...
            /// </summary>
            private bool claimDefault { get; }

            public FileAssociation(string extension, LocalisableString description, string iconPath, bool claimDefault = false)
            {
                Extension = extension;
                this.description = description;
                this.iconPath = iconPath;
                this.claimDefault = claimDefault;
            }
            ...
            public void Install()
            {
                ...
                using (var extensionKey = classes.CreateSubKey(Extension))
                {
                    if (claimDefault)
                    {
                        // Extensions we own outright: point the default ProgID at ours so Explorer
                        // draws our icon (it resolves the file icon through the extension's default
                        // ProgID, not the OpenWithProgIds list).
                        extensionKey.SetValue(null, ProgramId);
                    }
                    else if (extensionKey.GetValue(null) is string s && s == ProgramId)
                    {
                        // Shared formats: clear out any stale default of ours. Default programs in
                        // Windows are handled internally by Explorer, so forcing it here would just
            ...
            public void Uninstall()
            {
                ...
                {
                    // Release the default handler if we claimed it (.typb), so uninstall leaves no
                    // dangling ProgID reference behind.
