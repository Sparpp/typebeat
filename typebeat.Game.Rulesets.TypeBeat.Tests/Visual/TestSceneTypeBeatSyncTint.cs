...
namespace typebeat.Game.Rulesets.TypeBeat.Tests.Visual
{
    ...
    public partial class TestSceneTypeBeatSyncTint : OsuTestScene
    {
        ...
        /// at: q = 1 - GoodLate/OkLate, and the two scale together, so this is 0.5 on every tier.</summary>
        private double halfQualityLateDelta(int index) => SyncWindows.For(cell(index).JudgeGranularity).GoodLate;

        /// <summary>Far enough past the Ok window that sync quality is pinned at 0: the case that
        /// would render as the untyped grey if the ramp had no floor.</summary>
        private double hopelesslyLateDelta(int index) => SyncWindows.For(cell(index).JudgeGranularity).OkLate * 3;
