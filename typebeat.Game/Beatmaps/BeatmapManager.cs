...
namespace typebeat.Game.Beatmaps
{
    ...
    public class BeatmapManager : ModelManager<BeatmapSetInfo>, IModelImporter<BeatmapSetInfo>, IWorkingBeatmapCache
    {
        ...
        // Central beatmap file store — used to read a difficulty's previously-saved .osu back during
        // save() so a cosmetic-only edit can be detected (see the substantive check there).
        private readonly RealmFileStore beatmapFiles;
            ...
            : base(storage, realm)
        {
            ...
            beatmapFiles = new RealmFileStore(realm, storage);
            var userResources = beatmapFiles.Store;
        ...
        /// <summary>
        /// Perform a lookup query on available <see cref="BeatmapInfo"/>s.
        /// Use this overload instead of <see cref="QueryBeatmap(System.Linq.Expressions.Expression{System.Func{typebeat.Game.Beatmaps.BeatmapInfo,bool}})"/>
        ...
        /// <summary>Decodes UTF-8 bytes to text, stripping a leading byte-order mark if present, so a
        /// stored .osu and a freshly-encoded one compare consistently regardless of BOM.</summary>
        ...
        private void save(BeatmapInfo beatmapInfo, IBeatmap beatmapContent, ISkin? beatmapSkin, Storyboard? storyboard, bool transferCollections)
        {
            ...
            // A save normally demotes the map to LocallyModified. But a native-format ruleset can carry
            // menu/cosmetic-only fields (type!beat's intro beatdrop) whose edit must NOT un-rank the map.
            // The demotion is therefore deferred until we've compared the new encoding against the stored
            // one inside the write (see the `substantive` check); capture the pre-save status to restore.
            var previousBeatmapStatus = beatmapInfo.Status;
            var previousSetStatus = setInfo.Status;
            ...
            Realm.Write(r =>
            {
                ...
                // Whether this save substantively changes the map for ONLINE-STATUS purposes. For a
                // native-format ruleset we ask it to compare the new encoding against the stored one
                // ignoring cosmetic-only fields (type!beat's beatdrop); anything else — or a first save
                // with no stored file — is substantive and demotes as usual. Read the previously-stored
                // file now, before it's deleted just below.
                bool substantive = true;

                if (rulesetInstance.CanEncodeToNativeFormat && existingFileInfo != null)
                {
                    string? oldText = null;

                    try
                    {
                        using var oldStream = beatmapFiles.Storage.GetStream(existingFileInfo.File.GetStoragePath());

                        if (oldStream != null)
                        {
                            using var reader = new StreamReader(oldStream, Encoding.UTF8);
                            oldText = reader.ReadToEnd();
                        }
                    }
                    catch
                    {
                        // Unreadable previous file → treat as substantive (demote, the safe default).
                        oldText = null;
                    }

                    if (oldText != null)
                        substantive = !rulesetInstance.NativeEncodingsEquivalentForStatus(oldText, decodeUtf8(stream.ToArray()));
                }

                beatmapInfo.Status = substantive ? BeatmapOnlineStatus.LocallyModified : previousBeatmapStatus;
                ...
                // Recompute the set hash; demote the set to LocallyModified only for substantive changes
                // (a beatdrop-only edit keeps the set's prior online status, e.g. Ranked).
                setInfo.Hash = beatmapImporter.ComputeHash(setInfo);
                setInfo.Status = substantive ? BeatmapOnlineStatus.LocallyModified : previousSetStatus;
