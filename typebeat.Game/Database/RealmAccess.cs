...
namespace typebeat.Game.Database
{
    ...
    public class RealmAccess : IDisposable
    {
        ...
        private void attemptRecoverFromFile(string recoveryFilename)
        {
            ...
            try
            {
                using (var realm = Realm.GetInstance(getConfiguration()))
                {
                    if (realm.All<ScoreInfo>().Any())
                    {
                        ...
                        Logger.Log($@"To perform recovery, delete {OsuGameBase.CLIENT_DATABASE_FILENAME} while type!beat is not running.", LoggingTarget.Database);
                        return;
                    }
                }
            }
            ...
            {
                Logger.Log(@"Recovery aborted as the newer version could not be loaded by this type!beat version.", LoggingTarget.Database);
                return;
            }
        ...
        private Realm prepareFirstRealmAccess()
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Logger.Error(e, "Your local database is too new to work with this version of type!beat. Please close type!beat and install the latest release to recover your data.");
        ...
        /// </summary>
        /// <remarks>
        /// This adds type!beat specific thread and managed state safety checks on top of <see cref="IRealmCollection{T}.SubscribeForNotifications"/>.
        ///
        ...
        {
            // This is currently the only usage of temporary files at the type!beat side.
        ...
        private void applyMigrationsForVersion(Migration migration, ulong targetVersion)
        {
            ...
            switch (targetVersion)
            {
                ...
                case 42:
                    ...
                    {
                        // https://github.com/ppy/osu/blob/8773c2f7ebc226942d6124eb95c07a83934272ea/typebeat.Game.Rulesets.Mania/ManiaRuleset.cs#L327-L336
                        ...
                        // https://github.com/ppy/osu/blob/8773c2f7ebc226942d6124eb95c07a83934272ea/typebeat.Game.Rulesets.Mania/ManiaInputManager.cs#L22-L31
                ...
                case 46:
                {
                    // These migrations previously adjusted editor beat snap divisor key bindings, which no longer exist.
                    break;
                }
