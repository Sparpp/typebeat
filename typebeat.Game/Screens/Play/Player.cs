...
using typebeat.Game.Rulesets.UI.Scrolling;
...
namespace typebeat.Game.Screens.Play
{
    ...
    public abstract partial class Player : ScreenWithBeatmapBackground, ISamplePlaybackDisabler, ILocalUserPlayInfo
    {
            ...
            Children = new Drawable[]
            {
                DrawableRuleset.With(r =>
                    r.FrameStableComponents.Children = new Drawable[]
                    {
                        ScoreProcessor,
                        HealthProcessor,
                        // type!beat: ComboEffects (combo break sample) intentionally omitted.
                        breakTracker,
                    }),
            }
        };
        ...
        {
            // Matching type!beatstable behaviour, if the results screen is pending and the user requests an exit,
